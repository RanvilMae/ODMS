...
<?php
			...
			foreach ( $nquery as $get){
    			...
    			$id = $get['id'];
					...
					$query="select * from files WHERE 
							docu_id like '%".$search."%' AND id = '$id'
							OR subject like '%".$search."%'  AND id = '$id'
							ORDER BY id DESC" ;
  ...
  <tr>
        	...
        	<td style="color: black;"><?php echo $file['docu_id']; ?></td>
				  <td style="color: black;"><?php echo $file['fromw']; ?><br>
			...
			<td align="center">
				...
				<button type="button" class="btn btn-primary" title="PREVIEW">
										<a href="uploads/<?php echo $get['department']?>/<?php echo $file['name']?>" target="_blank" class="text-decoration-none" style="color:white;">
					...
					else
					{
						?>
					  	...
					  	<button type="button" class="btn btn-primary" title="VIEW">
										<a data-toggle="modal" data-target="#qr<?php echo $file['id']?>" style="text-decoration-none;color:#ff8a80;">
											<i class="fas fa-eye"></i>
										</a>
									</button>

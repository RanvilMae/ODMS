	...
	$id = (int) $_SESSION['m'];
			if(isset($_POST['search'])){
				...
				$tid = $fetch['tid'];
				   ...
				   $query = "SELECT * FROM tagto_personnels WHERE tag = '$tid'";
					$resultset = mysqli_query($conn, $query) or die("database error:". mysqli_error($conn));
				while($fetch = mysqli_fetch_assoc($resultset)) {
					$id = $fetch['id'];
					$primary_id = $fetch['primary_id'];
				$track= $fetch['track'];
			$search=$_POST['search'];
			$query="select * from files WHERE 
			`id` = '$id' AND department = '$department' AND docu_id like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND name like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND forw like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND fromw like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND subject like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND date like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND classification like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND category like '%".$search."%' OR
			`id` = '$id' AND department = '$department' AND restriction like '%".$search."%' ORDER BY id DESC";
      ...
      <div class="modal-dialog modal-lg">
        ...
        <h3 class="modal-title">HISTORY</h3>
		  ...
		  <a href="download_status.php?id=<?php echo $id?>" target="_blank" class="text-decoration-none">
				...
				<a href="remarks_add_user.php?id=<?php echo $id?>" class="text-decoration-none">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
				<button type="button" name="add" class="btn btn-primary" title="ADD TAG">
						<a href="add_tag_m.php?id=<?php echo $id?>" class="text-decoration-none" style="text-decoration-none;color:white;>" >

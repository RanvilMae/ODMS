...
include('conn.php');
		...
		$query = $conn->query ("SELECT department FROM centraladmin WHERE id = '$id' ") or die (mysqli_error());
		$get = $query->fetch_array ();
		$department = $get['department'];
		$nquery=mysqli_query($conn,"select * from `files` WHERE department = '$department'  ORDER by id DESC ");
...
include('conn.php');
  ...
  <p><font face="verdana" color="green" class="float-left"><strong><?php echo $subject?></strong></font></p>
            ...
            <table class="table table-bordered">
			  ...
			  echo "<td width='10%'>". $fetch['remarks'] ."</td>";
			...
			<div class="modal-footer">
				...
				<a href="csubfile.php?id=<?php echo $id?>" class="text-decoration-none">
...
include('conn.php');
  ...
  <p><font face="verdana" color="green" class="float-left"><strong><?php echo $subject?></strong></font></p>
			...
			<div class="modal-footer">
				...
				<a href="cremarks.php?id=<?php echo $id?>" class="text-decoration-none">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
				...
				<button type="button" name="delete" class="btn btn-secondary " title="REMOVE TAG">
						<a href="remove_tagc.php?id=<?php echo $id?>" class="text-decoration-none" style="text-decoration-none;color:white;>" >
							<i class="fas fa-trash-alt"></i>
						</a>
				</button>
				...
				<button type="button" name="add" class="btn btn-primary" title="ADD TAG">
						<a href="add_tag_ca.php?id=<?php echo $id?>" class="text-decoration-none" style="text-decoration-none;color:white;>" >
...
if (isset($_POST['save'])) {
			...
			$category=$_POST['category'];
					mysqli_query($conn, "UPDATE `files` SET `forw` = '$forw', `for_specific` = '$for_specific', `fromw` = '$fromw', `from_specific` = '$from_specific', `pages` = '$pages', `subject` = '$subject', `date` = '$date', `category` = '$category', `restriction` = 'Outgoing' WHERE `id` = '$id'") ;
			...
			$category=$_POST['category'];
					mysqli_query($conn, "UPDATE `files` SET `forw` = '$forw', `for_specific` = '$for_specific', `fromw` = '$fromw', `from_specific` = '$from_specific',`pages` = '$pages', `subject` = '$subject', `date` = '$date', `restriction` = '$restriction', `category` = '$category' WHERE `id` = '$id'") ;
					$message = "FILE SUCCESSFULLY UPDATED!";
					echo "<script type='text/javascript'>alert('$message');</script>";
					echo "<script>window.location = 'ca_viewdata.php'</script>";
		...
		if (isset($_POST['save']))
	{
		...
		$destination = "cuploads/";

		// overwrite file

		//checking if file exsists
		if(file_exists("cuploads/$filename")) unlink("cuploads/$filename");

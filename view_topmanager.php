...
<?php
    ...
    $result = mysqli_query($conn,"SELECT * FROM `files` WHERE department = '$department' ORDER by id DESC LIMIT 200 ");
...
include('conn.php');
		...
		$tid = $file['tid'];
		$department = $file['department'];
    $result = mysqli_query($conn,"SELECT * FROM `files` WHERE department = '$department' ORDER by id DESC LIMIT 100 ");
    foreach ( $result as $fetch){
		$did = $fetch['id'];
		$subject = $fetch['subject'];
							...
							$run_num_rows = $query->num_rows;
								if ($run_num_rows > 0 )
									...
									else
									{
										...
										while($fetchh = mysqli_fetch_array($query))
										{
											echo "<td width='30%'>". $fetchh['lname'] .", ". $fetchh['fname'] ." ". $fetch['mname'] ." - ". $fetchh['department'] ."</td>";
										}
											echo "<td width='20%'>". $get['date'] ."</td>";
											echo "<td width='10%'> VIEWED</td>";
											echo "<td width='20%'>". $get['dateviewed'] ."</td>";

...
include('conn.php');
		...
		$nquery=mysqli_query($conn,"select * from `files` WHERE department = '$department' ");
		foreach ( $nquery as $gett)
				{
			$id = $gett['id'];
			$query=mysqli_query($conn,"select * from `subfiles` WHERE id = '$id' ORDER by id DESC");
		foreach ( $query as $fetchh)
				{
			$id = $fetchh['id'];

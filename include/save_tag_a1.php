...
<?php 
	...
	if(isset($_POST['save']))  
	{
		...
		for($i=0; $i < $N; $i++)
		{
			$check 		= mysqli_query($conn,"SELECT * FROM tags WHERE id = '$id' AND tag = '$tag[$i]'");
		...
		if($error == 0)
		{
			...
			echo "<script type='text/javascript'>alert('PLEASE FILL IN DOCUMENT STATUS');</script>";
			echo "<script>window.location = 'remarks1.php?id=".$id."'</script>";
		}
	}
?>

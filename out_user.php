...
include('conn.php');
		...
		$department = $fetch['department'];
					$nquery=mysqli_query($conn,"select * from `tagto_personnels` WHERE tag = '$tid'  ORDER BY date DESC ");
			foreach ( $nquery as $get){
			 $track = $get['track'];
			 $primary_id = $get['primary_id'];
			 $id = $get['id'];
    $result = mysqli_query($conn,"SELECT * FROM `files` WHERE restriction = 'Outgoing' AND  id = '$id' ORDER by date DESC  ");
...
include('conn.php');
		...
		$department = $fetch['department'];
			...
			foreach ( $nquery as $get){
			 ...
			 $primary_id = $get['primary_id'];
			 $id = $get['id'];
    $result = mysqli_query($conn,"SELECT * FROM `files` WHERE restriction = 'Outgoing' AND  id = '$id' ORDER by id DESC  ");
    foreach ( $result as $fetch){
		$did = $fetch['id'];
		$subject = $fetch['subject'];

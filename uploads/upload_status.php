  ...
  <div class="jumbotron">
    <form action="upload_status.php" method="post" enctype="multipart/form-data" >
      <div class="form-group">
           <h3><strong>ADD STATUS</strong></h3>
          <hr>
          <br>
		
		<label><strong>STATUS: </strong></label>
		<input type="text" class="form-control" placeholder="Status" id="status" name="status" required /><br>
 ...
 <div style="overflow-x:auto;">
    ...
    <tbody>
		<?php
			...
			$nquery=mysqli_query($conn,"select * from `status` ORDER BY id DESC ");
        ...
        <tr>
			...
			<td align="center">
			<button type="button" name="delete" value="C:\xampp\htdocs\ss\archive<?php echo $file['status']?>" class="btn btn-danger" title="DELETE">
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			Are you sure you want to delete <strong><?php echo $file['status'] ?></strong>?
			<div class="modal-footer">
         <a href="delete_stat.php?status=<?php echo $file['status'] ?>"   class="btn btn-danger"><i class="fas fa-trash-alt"></i></a>
	...
	if (isset($_POST['sub']))
{
	...
	$query = $conn->query("SELECT * FROM `status` WHERE `status` = '$status'");
	$check = $query->num_rows;
		
		if($check == 1)
			{
				echo "<script>alert('STATUS ALREADY EXIST!')</script>";	 
			}
			
			else
				{
					$conn->query ("INSERT INTO status (status)
					VALUES ('$status')") or die(mysqli_error());	
					echo "<script>alert('STATUS SAVED!')</script>"; 

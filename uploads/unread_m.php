...
.notif .badges {
 ...
 <main role="main" class="container-fluid">
  ...
  <div class="float-right"> 
    <a href="search_manager.php" class="text-decoration-none">
    <button type="button" title="SEARCH" class="btn btn-primary" title="SEARCH" ><i class="fas fa-search"></i></button>
      </button></a>
...
include('conn.php');
		...
		$department = $fetch['department'];
					$nquery=mysqli_query($conn,"select * from `tagto_personnels` WHERE tag = '$tid' AND track='0' ORDER BY id DESC  ");
			foreach ( $nquery as $get){
			 $track = $get['track'];
			 $primary_id = $get['primary_id'];
			 $id = $get['id'];
    $result = mysqli_query($conn,"SELECT * FROM `files` WHERE id = '$id' ORDER by id DESC  ");
